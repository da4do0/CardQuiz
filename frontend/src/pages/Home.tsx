        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            ...
            <nav className="hidden md:flex space-x-8">
              ...
              <Link to="/create-quiz" className="text-white hover:text-purple-200 transition-colors">
                Create Quiz
              </Link>
              <Link to="/my-quizzes" className="text-white hover:text-purple-200 transition-colors">
                My Quizzes
              </Link>
            </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-white/80 mb-8 max-w-3xl mx-auto">
            Discover amazing quizzes, test your knowledge, and compete with friends. 
            Create custom quizzes or choose from our extensive library.
          </p>
        ...
        {/* Featured Quizzes */}
        <section>
          <h3 className="text-3xl font-bold text-white mb-8 text-center">Featured Quizzes</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredQuizzes.map((quiz) => (
              <div key={quiz.id} className="card hover:scale-105 transition-transform duration-200">
                <div className="mb-4">
                  <span className="inline-block px-3 py-1 bg-purple-500/30 text-purple-200 text-sm rounded-full mb-2">
                    {quiz.category}
                  </span>
                  <h4 className="text-xl font-semibold text-white mb-2">{quiz.title}</h4>
                  <p className="text-white/80 text-sm mb-4 line-clamp-3">{quiz.description}</p>
                </div>
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center space-x-4 text-sm text-white/70">
                    <span>⏱️ {quiz.timeLimit ? formatTime(quiz.timeLimit) : 'No limit'}</span>
                    <span>📊 {Math.floor(Math.random() * 1000) + 100} plays</span>
                  </div>
                </div>
                <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {[
              ...
              >
                ...
                <div className="text-white/60 text-sm">{category.count} quizzes</div>
              </Link>
            ))}
          </div>
        </section>
